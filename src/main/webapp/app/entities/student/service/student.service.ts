...
export class StudentService {
  ...
  protected convertDateFromClient(student: IStudent): IStudent {
    return Object.assign({}, student, {
      dateOfBirthday: student.dateOfBirthday?.isValid() ? student.dateOfBirthday.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((student: IStudent) => {
        student.dateOfBirthday = student.dateOfBirthday ? dayjs(student.dateOfBirthday) : undefined;
      });
    }
    return res;
  }
