...
class ProviderResourceIT {
    ...
    @Test
    @Transactional
    void getAllProvidersByNameIsNotEqualToSomething() throws Exception {
        // Initialize the database
        providerRepository.saveAndFlush(provider);

        // Get all the providerList where name not equals to DEFAULT_NAME
        defaultProviderShouldNotBeFound("name.notEquals=" + DEFAULT_NAME);

        // Get all the providerList where name not equals to UPDATED_NAME
        defaultProviderShouldBeFound("name.notEquals=" + UPDATED_NAME);
    }
    ...
    void putWithMissingIdPathParamProvider() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restProviderMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(providerDTO)))
    ...
    void patchWithMissingIdPathParamProvider() throws Exception {
        ...
        restProviderMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(providerDTO))
            )
            .andExpect(status().isMethodNotAllowed());
